...
const saveToLocalStorage = (key: string, value: any) => {
  try {
    // Block saving steps or currentStep to localStorage to prevent them from persisting
    if (key === 'steps' || key === 'currentStep') {
      console.log(`Blocked saving ${key} to localStorage - steps should be generated by user`);
      return;
    }
...
const loadFromLocalStorage = (key: string, defaultValue: any = null) => {
  try {
    // Always return defaults for steps and currentStep to ensure they're never loaded from localStorage
    if (key === 'steps') {
      console.log('Blocked loading steps from localStorage - always returning empty array');
      return [];
    }
    
    if (key === 'currentStep') {
      console.log('Blocked loading currentStep from localStorage - always returning 0');
      return 0;
    }
...
function App() {
  ...
  const [currentStep, setCurrentStep] = useState<number>(0); // Always start at 0, ignore localStorage
  const [isLooping, setIsLooping] = useState<boolean>(loadFromLocalStorage('isLooping', true));
  const [steps, setSteps] = useState<Step[]>([]); // ALWAYS start with empty steps, ignore localStorage
  ...
  useEffect(() => {
    ...
    videoId, audioWithClicksUrl, harmonicWithClicksUrl, percussiveWithClicksUrl,
    harmonicOriginalUrl, percussiveOriginalUrl, clicksOnlyUrl, waveformImage, isDummyData, beats, downbeats, videoDuration,
  ...
  useEffect(() => {
    ...
    if (videoId && !isLoading && beats.length === 0) {
  ...
  const fetchProgress = async (videoIdToCheck: string) => {
    try {
      ...
      if (response.ok) {
        ...
        // IMPORTANT: Pre-filter the response to remove any steps
        if (data.steps) {
          console.log('PROGRESS POLL: Removing auto-generated steps from response');
          delete data.steps;
        }
        ...
        if (data.videoId && !('progress' in data)) {
          ...
          setProgress(100);
          setStatusMessage('Analysis complete - use the timeline editor to generate steps');
  ...
  const directApiCall = async (endpoint: string, options: RequestInit = {}) => {
    try {
      ...
      // IMPORTANT: Filter out any steps from the API response to ensure user must generate them manually
      if (responseData && responseData.steps) {
        console.log('Removing auto-generated steps from API response');
        delete responseData.steps;
      }
    ...
    try {
      ...
      if (progressData) {
        ...
        if (typeof progressData.progress === 'number') {
          ...
          // Check if processing is complete - we handle beats and downbeats separately
          if (progressData.progress >= 100) {
            // Don't use the automatic steps - call handleProcessingComplete to just get beats/downbeats/etc
            handleProcessingComplete(progressData);
          }
        } 
        // Otherwise, we might have the full result but we don't auto-set steps
        else if (progressData.beats || progressData.downbeats) {
          // Only get beats/downbeats/etc from the completed data
          handleProcessingComplete(progressData);
        }
      }
    } catch (error) {
  ...
  const handleProcessingComplete = (data: any) => {
    ...
    // IMPORTANT: Don't set steps automatically - user will generate them via the timeline editor
    // if (data.steps) setSteps(data.steps);
    
    if (data.duration) setVideoDuration(data.duration);
    if (data.beats) setBeats(data.beats);
    if (data.downbeats) setDownbeats(data.downbeats);
    ...
    // No automatic navigation to steps since we're not setting steps automatically
    // Instead, user will generate and navigate to steps via the timeline editor
  ...
  const handleNewVideo = () => {
    ...
    // Reset state
    setVideoId('');
    setSteps([]); // Ensure steps is reset to empty array
  ...
  // Initialize app state by clearing any preloaded or cached steps
  useEffect(() => {
    // Always ensure steps start as empty array, regardless of localStorage
    setSteps([]);
    // Clear step-related localStorage to ensure we don't reload autoamtic steps on refresh
        ...
        <MainContent>
          ...
          {videoId && !isLoading && (
            <>
              ...
              {/* Step information and timeline - only show when steps have been generated */}
              {steps.length > 0 && (
                <>
                  {/* Current step info */}
                  <StepInfo>
              ...
              {/* No steps generated message - show when beats/downbeats are available but no steps yet */}
              {beats.length > 0 && steps.length === 0 && (
                <div style={{ 
                  textAlign: 'center', 
                  margin: '20px 0', 
                  padding: '15px', 
                  ...
                  <h3 style={{ color: '#1DB954' }}>Analysis Complete!</h3>
                  <p>Use the timeline editor below to generate dance steps:</p>
                  <ol style={{ textAlign: 'left', display: 'inline-block' }}>
                    <li>Select a starting downbeat</li>
                    <li>Choose a stride (how many beats per step)</li>
                    <li>Click "Generate Steps" to create your dance sequence</li>
                  </ol>
                </div>
              )}
